...
static int          ft_read_all(const int fd, char **str, int size, int sizeall)
{
    char    *temp;
...
static t_lists      *ft_get_curx(t_lists **lists, const int fd, t_lists *cur)
{
    if ((*lists) == NULL)
    {
...
int                 get_next_line(const int fd, char **line)
{
    ...
    if ((bufsize = ft_read_all(cur->fd, &str, 0, 0)) == -1)
        RETRUN_ERROR(-1);
    if (bufsize)
    {
